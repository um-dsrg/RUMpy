...
def test_supmoco(dummy_image, tmp_path):  # tmp_path is a temporary location which is deleted after running
    internal_params = {'name': 'supmoco', 'internal_params': {'crop_count': 4,
                                                              'model_name': "default"}}

    model = contrastive_model_setup(internal_params, directory=tmp_path)
    result, _, _ = model.net_run_and_process(dummy_image)

    assert result[0].shape == (1, 256)


def test_moco(dummy_image, tmp_path):  # tmp_path is a temporary location which is deleted after running
...
def test_weakcon(dummy_image, tmp_path):  # tmp_path is a temporary location which is deleted after running
