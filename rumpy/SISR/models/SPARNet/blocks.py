...
class ResidualBlock(nn.Module):
    ...
    # Added modifications to include metadata.
    """
    def __init__(self, c_in, c_out, relu_type='prelu', norm_type='bn', scale='none', hg_depth=2, att_name='spar',
        ...
        if include_metadata is not None:
            ...
            self.metadata_attention = ParaCALayer(network_channels=self.c_out, num_metadata=include_metadata, nonlinearity=True)
        else:
    ...
    def forward(self, x):
        ...
        if type(x) == tuple:
            ...
            data = x
        identity = self.shortcut_func(data)
        out = self.preact_func(data)
        out = self.conv1(out)
        out = self.conv2(out)
        out = identity + self.att_func(out)
        if type(x) == tuple:
            if self.include_metadata:
                out = self.metadata_attention(out, x[1])
            return out, x[1]
        else:
            return out
